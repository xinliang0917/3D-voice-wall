...
const directFonts = [
  // CFF outlines avoid three-text filling errors on overlapping CJK glyphs.
  [
    'https://cdn.jsdelivr.net/gh/notofonts/noto-cjk@main/Sans/SubsetOTF/SC/NotoSansSC-Bold.otf',
    'noto-sans-sc-chinese-simplified-700-normal.otf',
  ],
];
...
for (const [url, fileName] of directFonts) {
  const response = await fetch(url);
  if (!response.ok) {
    throw new Error(`Failed to download ${url}: ${response.status}`);
  }
